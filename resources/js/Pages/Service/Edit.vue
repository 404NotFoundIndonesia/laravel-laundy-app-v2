...
import {digitFormatter, validateNumber} from "@/util.js";

const page = usePage();

const form = useForm({
    name: page.props.service.name,
    unit: page.props.service.unit ?? page.props.units[0],
    original_price: digitFormatter(page.props.service.original_price),
    sale_price: digitFormatter(page.props.service.sale_price),
});
...
const submit = () => {
    ...
    })).put(route('service.update', page.props.service.id), {
        preserveScroll: true,
        preserveState: true,
    });
};
        ...
        <template #header>
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-2 rtl:space-x-reverse">
                    ...
                    <li>
                        <Link :href="route('service.show', $page.props.service.id)">
                            <h2 class="font-semibold text-xl text-gray-400 hover:text-gray-600 leading-tight">
