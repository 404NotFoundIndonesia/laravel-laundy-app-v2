...
const destroy = (id) => {
    deleteConfirmation(() => {
        const destroyRoute = typeof id === 'object' ?
            route('customer.destroy-bulk') + '?' + (new URLSearchParams({ids: selected.value}).toString()) :
    ...
    <AppLayout title="Customer">
                                ...
                                <template v-if="$page.props.items?.data?.length">
                                    <tr class="border-b dark:border-gray-600" v-for="customer in $page.props.items.data"
                                        ...
                                        <td class="w-4 px-4 py-3">
                                            <div class="flex items-center">
                                                <input :id="`checkbox-table-search-${customer.id}`" v-model="selected"
                                                       :value="customer.id" type="checkbox"
        ...
        <nav v-show="selected.length > 0" class="fixed bottom-0 w-full bg-white shadow-xl z-50 dark:bg-gray-800">
            <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex justify-between items-center">
                ...
                <h5>{{ selected.length }} {{ selected.length > 1 ? 'customers' : 'customer' }} selected.</h5>
            </div>
        </nav>
    </AppLayout>
</template>
