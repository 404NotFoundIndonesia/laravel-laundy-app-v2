...
import {dateHumanFormat} from "@/util.js";
                ...
                <div class="bg-white shadow-xl sm:rounded-lg">
                    <div class="py-6 mx-auto px-6 2xl:py-16 2xl:px-16">
                        <div class="flex items-center justify-between">
                            <div>
                                <h3 class="mb-2 text-xl font-semibold leading-none text-gray-900 md:text-2xl dark:text-white">
                                    {{ $page.props.supplier.name }}</h3>
                                <small class="text-gray-500">Our supplier since:
                                    {{ dateHumanFormat($page.props.supplier.created_at) }}</small>
                            </div>
                            <div class="px-4 py-3 flex items-center justify-end">
                                <button :id="`supplier-id-${$page.props.supplier.id}`"
                                        data-dropdown-placement="bottom-end"
                                        :data-dropdown-toggle="`supplier-id-${$page.props.supplier.id}-dropdown`"
                                ...
                                <div :id="`supplier-id-${$page.props.supplier.id}-dropdown`"
                                     class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                        :aria-labelledby="`supplier-id-${$page.props.supplier.id}`">
                                        <li>
                                            <Link :href="route('supplier.edit', $page.props.supplier.id)"
                        ...
                        <dl class="flex items-center space-x-6">
                            <div v-if="$page.props.supplier.email">
                                <dt class="mb-1 text-sm font-semibold leading-none text-gray-600 dark:text-white">
                                    Email
                                </dt>
                                <dd class="mb-4 text-sm font-light text-gray-500 sm:mb-5 dark:text-gray-400">
                                    {{ $page.props.supplier.email }}
                                </dd>
                            </div>
                            <div class="me-5">
                                <dt class="mb-1 text-sm font-semibold leading-none text-gray-600 dark:text-white">
                            ...
                            <div class="me-5">
                                ...
                                <dd class="mb-4 text-sm font-light text-gray-500 sm:mb-5 dark:text-gray-400">
                                    {{ $page.props.supplier.contact_person }}
                                </dd>
                            </div>
                        </dl>
                        <dl>
                            <dt class="mb-1 text-sm font-semibold leading-none text-gray-600 dark:text-white">Address
                            </dt>
                            <dd class="mb-4 text-sm font-light text-gray-500 sm:mb-5 dark:text-gray-400">
                                {{ $page.props.supplier.address ?? '-' }}
                            </dd>
                        </dl>
                    </div>
                </div>
