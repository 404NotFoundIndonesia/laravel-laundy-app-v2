...
const filter = () => {
    filterForm.get(route('transaction.index'), {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const destroy = (id) => {
    deleteConfirmation(() => {
        const destroyRoute = typeof id === 'object' ?
            route('transaction.destroy-bulk') + '?' + (new URLSearchParams({ids: selected.value}).toString()) :
    ...
    <AppLayout title="Transaction">
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white shadow-xl sm:rounded-lg">
                    <div class="relative bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
                        ...
                        <div class="px-4 py-3">
                            <h5>
                                <span class="text-gray-500 me-2">Total Transactions:</span>
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                ...
                                <template v-if="$page.props.items?.data?.length">
                                    <tr class="border-b dark:border-gray-600" v-for="transaction in $page.props.items.data"
                                        :key="transaction.id">
                                        <td class="w-4 px-4 py-3">
                                            <div class="flex items-center">
                                                <input :id="`checkbox-table-search-${transaction.id}`" v-model="selected"
                                                       :value="transaction.id" type="checkbox"
                                                       onclick="event.stopPropagation()"
                                                       class="w-4 h-4 bg-gray-100 border-gray-300 rounded text-emerald-600 focus:ring-emerald-500 dark:focus:ring-emerald-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                                                <label :for="`checkbox-table-search-${transaction.id}`" class="sr-only">checkbox</label>
                                            </div>
                                        </td>
                                        <td class="px-4 py-2 font-regular text-gray-900 whitespace-nowrap dark:text-white">
                                            {{ dateHumanFormat(transaction.created_at) }}
                                        </td>
                                        ...
                                        <td class="px-4 py-2 text-gray-900 whitespace-nowrap dark:text-white">
                                            <template v-if="transaction.original_price === transaction.sale_price || transaction.sale_price === 0">
                                                {{ rupiahCurrency(transaction.original_price) }}
                                            </template>
                                            <div v-else>
                                                <small class="text-gray-500 line-through">{{ rupiahCurrency(transaction.original_price) }}</small>
                                                <div>{{ rupiahCurrency(transaction.sale_price) }}</div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 flex items-center justify-end">
                                            <button :id="`transaction-id-${transaction.id}`"
                                                    data-dropdown-placement="bottom-end"
                                                    :data-dropdown-toggle="`transaction-id-${transaction.id}-dropdown`"
                                            ...
                                            <div :id="`transaction-id-${transaction.id}-dropdown`"
                                                 class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                                <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                                    :aria-labelledby="`transaction-id-${transaction.id}`">
                                                    <li>
                                                ...
                                                <div class="py-1">
                                                    <a href="#" @click="() => destroy(transaction.id)"
        ...
        <nav v-show="selected.length > 0" class="fixed bottom-0 w-full bg-white shadow-xl z-50 dark:bg-gray-800">
            <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex justify-between items-center">
                ...
                <h5>{{ selected.length }} {{ selected.length > 1 ? 'transactions' : 'transaction' }} selected.</h5>
            </div>
        </nav>
    </AppLayout>
</template>
